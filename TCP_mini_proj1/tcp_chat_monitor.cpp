...
int main(int argc, char *argv[]) {
	...
	if (nickname != NULL) {
		...
		memcpy(&send_buf[offset], &mon_connect, sizeof(ChatMonMsg));
		offset += sizeof(ChatMonMsg);
		...
		if (FD_ISSET(monitor_socket, &read_set)) {
			...
			if (ret >= sizeof(struct ChatMonMsg)) {
				...
				if (server_message.type == MON_MESSAGE) {
					...
					char *temp_data_buf = (char *) malloc(server_message.data_len);
					char *temp_name_buf = (char *) malloc(server_message.nickname_len);
					memset(temp_data_buf, 0, sizeof(ChatMonMsg));
					if (ret >= offset + server_message.nickname_len) {
						memcpy(temp_data_buf, &recv_buf[offset], server_message.nickname_len);
						offset += server_message.nickname_len;
						std::cout << "Received nickname " << temp_name_buf << std::endl;
						memcpy(temp_data_buf, &recv_buf[offset], server_message.data_len);
						std::cout << "Data received is" << temp_data_buf << std::endl;
	...
	malloc(sizeof(ChatMonMsg));
	memset(mon_disconnect, 0, sizeof(ChatMonMsg));
	mon_disconnect->type = htons(MON_DISCONNECT);
	mon_disconnect->data_len = htons(sizeof(ChatMonMsg));

	ret = send(monitor_socket, mon_disconnect, sizeof(ChatMonMsg), 0);
